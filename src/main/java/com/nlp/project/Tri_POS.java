...
public class Tri_POS {
	...
	public void createMap() {
		...
		for(int combination = 1; combination <= 5; combination++) {			
				...
				while((str = br.readLine()) != null) {
					if(!(str.isEmpty() || str.equals("### END ###"))) {
						...
						for(String oneLine : lines) {
							...
							String word;
							for(int x = 0; x < tags.length - 2; x++){
								hasSpecialChar = p.matcher(tags[x]).find();
								if(!hasSpecialChar && !p.matcher(tags[x + 1]).find()
										   && !p.matcher(tags[x + 2]).find()) {
									word = tags[x] + tags[x + 1] + tags[x + 2];
			...
			String training_file = "iphone-500/training-tripos-" + combination + ".arff";
			String test_file = "iphone-500/test-tripos-" + combination + ".arff";
			try{
